...
class Connector:
    ...
    async def _create_channel(self):
        ...
        log.info(f"Created gRPC channel to {self.target}")
    ...
    async def _monitor_channel_state(self):
        ...
        while not self._shutdown.is_set():
            ...
            log.debug(f"Channel state: {state.name}")
            if state == grpc.ChannelConnectivity.READY:
                ...
                if self._ready.is_set():
                    log.warning(f"gRPC channel is {state.name}. Blocking calls until READY.")
                    self._ready.clear()
        ...
        for attempt in range(1, self.retry_attempts + 1):
            try:
                ...
                if e.code() in RETRYABLE_CODES:
                    log.warning(f"RPC failed ({e.code().name}), retrying {attempt}/{self.retry_attempts}...")
