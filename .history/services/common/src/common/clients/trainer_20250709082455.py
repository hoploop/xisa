...
class TrainerClient(Client):
    ...
    async def trainImageObjectToDetector(self,user:User,detectorId:PydanticObjectId,recordId: PydanticObjectId) -> int:
        req = TrainImageObjectToDetectorRequest(user=str(user.id),detector=str(detectorId),record=str(recordId))
