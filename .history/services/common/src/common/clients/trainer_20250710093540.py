...
class TrainerClient(Client):
    ...
    async def trainSessionCreate(self,user:User,detectorId:PydanticObjectId)-> PydanticObjectId:
    ...
    async def trainSessionUpdate(self,user:User,sessionId:PydanticObjectId,status:TrainSessionStatus,error: Optional[str]=None) -> bool:
        ...
        if status==TrainSessionStatus.RUNNING:
            pstatus = PTrainSessionStatus.RUNNING
        elif status ==TrainSessionStatus.COMPLETED:
            pstatus = PTrainSessionStatus.COMPLETED
        elif status == TrainSessionStatus.ERROR:
            pstatus = PTrainSessionStatus.ERROR
        req = TrainSessionUpdateRequest(user=str(user.id),id=str(sessionId),status = status,error=error)
    ...
    async def trainSessionDetectorRunning(self,user:User,detectorId:PydanticObjectId) -> bool:
        req = TrainSessionDetectorRunningRequest(user=str(user.id),detector=str(detectorId))
        res: TrainSessionDetectorRunningResponse = await self.client.trainSessionDetectorRunning(req)
        if res.status == False:
            raise Exception(res.message)
        return res.running