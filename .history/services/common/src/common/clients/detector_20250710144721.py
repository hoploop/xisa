...
class DetectorClient(Client):
    ...
    async def trainResult(self,user:User,detectorId:PydanticObjectId,filename:str) -> TrainResultResponse:
        req = TrainResultRequest(user=str(user.id),detector=str(detectorId),filename=filename)
