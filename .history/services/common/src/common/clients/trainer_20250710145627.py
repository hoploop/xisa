...
class TrainerClient(Client):
    ...
    async def trainImageObjectList(
        ...
        recordId: PydanticObjectId,
        frame: int = -1,
    ) -> Tuple[int, List[TrainImageObject]]:
        req = TrainImageObjectListRequest(
            user=str(user.id),
            record=str(recordId),
            detector=str(detectorId),
            frame=frame,
    ...
    async def trainImageObjectRemove(
        self, user: User, objectId: PydanticObjectId
    ) -> bool:
        req = TrainImageObjectRemoveRequest(user=str(user.id), id=str(objectId))
        res: TrainImageObjectRemoveResponse = await self.client.trainImageObjectRemove(
            req
        )
        if res.status == False:
            raise Exception(res.message)
    ...
    async def trainImageObjectUpdate(
        self,
        user: User,
        imageObjectId: PydanticObjectId,
        labels: List[str],
        val: bool,
        test: bool,
        train: bool,
        ...
        req = TrainImageObjectUpdateRequest(
            user=str(user.id),
            id=str(imageObjectId),
            labels=labels,
            val=val,
            test=test,
            train=train,
        ...
        res: TrainImageObjectUpdateResponse = await self.client.trainImageObjectUpdate(
            req
        )
        if res.status == False:
            raise Exception(res.message)
    ...
    async def trainImageObjectCountByDetector(
        self, user: User, detectorId: PydanticObjectId
    ) -> int:
        req = TrainImageObjectCountByDetectorRequest(
            user=str(user.id), detector=str(detectorId)
        )
        res: TrainImageObjectCountByDetectorResponse = (
            await self.client.trainImageObjectCountByDetector(req)
        )
        if res.status == False:
            raise Exception(res.message)
        return res.total
    ...
    async def trainImageObjectToDetector(
        ...
        req = TrainImageObjectToDetectorRequest(
            user=str(user.id), detector=str(detectorId)
        )
        res: TrainImageObjectToDetectorResponse = (
            await self.client.trainImageObjectToDetector(req)
        )
        if res.status == False:
            raise Exception(res.message)
        return res.total
    ...
    async def trainSessionUpdate(
        ...
        log.debug("Updating session: "+status)
        req = TrainSessionUpdateRequest(
            user=str(user.id), id=str(sessionId), status=status, error=error, results=results
        )
    ...
    async def trainSessionList(
        self, user: User, detectorId: PydanticObjectId, skip: int = 0, limit: int = 10
    ) -> Tuple[int, List[TrainSession]]:
        req = TrainSessionListRequest(
    ...
    async def trainSessionDetectorRunning(
        ...
        req = TrainSessionDetectorRunningRequest(
            user=str(user.id), detector=str(detectorId)
        )
        res: TrainSessionDetectorRunningResponse = (
            await self.client.trainSessionDetectorRunning(req)
        )
        if res.status == False:
            raise Exception(res.message)
        return res.running

    async def trainSessionRemove(self,user:User,sessionId:PydanticObjectId) -> bool:
        req = TrainSessionRemoveRequest(user=str(user.id),id=str(sessionId))
        res: TrainSessionRemoveResponse = await self.client.trainSessionRemove(req)
        if res.status == False:
            raise Exception(res.message)
        return res.status