...
async def secure_channel_factory(security_config: SecurityConfig, client_config: ClientConfig, interceptors: Sequence[ClientInterceptor]|None=None,max_retries=5,retry_delay=3,timeout=3) -> grpc.aio.Channel:
...
class Connector:
    def __init__(
        ...
        *,
        ...
        """
        :param stub_class: gRPC Stub class (e.g. MyServiceStub)
        :param target: server host:port
        :param secure: True for SSL/TLS
        :param root_certificates: TLS root certs (optional)
        :param retry_attempts: RPC retry count
        :param retry_backoff: Base backoff seconds
        :param monitor_interval: Interval to check channel state
        """
        ...
        self.stub: stub_class | None = None
    ...
    async def _create_channel(self):
        if self.secure:
            ...
            self.channel = aio.secure_channel(self.target, creds)
        else:
    ...
    async def _monitor_channel_state(self):
        ...
        while not self._shutdown.is_set():
            ...
            logger.debug(f"Channel state: {state.name}")
            if state == grpc.ChannelConnectivity.READY:
                ...
                if self._ready.is_set():
                    logger.warning(f"gRPC channel is {state.name}. Blocking calls until READY.")
                    self._ready.clear()
    ...
    async def _wait_until_ready(self, timeout: float = 10.0):
        ...
        try:
            ...
            raise RuntimeError("gRPC channel not ready within timeout.")

    async def call_with_retry(self, rpc_call, *args, timeout: float = 10.0, **kwargs):
        """Call RPC with retries and channel readiness check"""
        ...
        for attempt in range(1, self.retry_attempts + 1):
            try:
                ...
                if e.code() in RETRYABLE_CODES:
                    logger.warning(f"RPC failed ({e.code().name}), retrying {attempt}/{self.retry_attempts}...")
                    await asyncio.sleep(self.retry_backoff * attempt)
                    continue
                else:
                    logger.error(f"RPC failed (non-retryable): {e.code().name}")
                    raise
