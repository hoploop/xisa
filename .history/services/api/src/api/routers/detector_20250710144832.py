...
@router.get("/results/{detectorId}/{filename}",operation_id="detectorTrainResult")
async def train_result(user: CurrentUser,detectorId:PydanticObjectId,filename: str, detector: Detector):
    try:
        res = await detector.trainResult(user,detectorId,filename)
