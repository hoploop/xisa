...
@router.post(
    ...
    operation_id="trainerSessionCreate",
...
@router.post(
    ...
    operation_id="trainerSessioUpdate",
    response_model=bool
 )
async def session_update(user:CurrentUser,trainer:Trainer,sessionId:PydanticObjectId,status:TrainSessionStatus,error:Optional[str]=None):
    try:
        return await trainer.trainSessionUpdate(user,sessionId,status,error)
    except Exception as e:
        raise HTTPException(status_code=status.HTTP_406_NOT_ACCEPTABLE, detail=str(e))
 
class TrainSessionListResponse(BaseModel):
    total: int
    sessions: List[TrainSession]
 
@router.get(
    "/session/list/{detectorId}",
    operation_id="trainerSessionList",
    response_model=TrainSessionListResponse,
)
async def session_list(
    user: CurrentUser, trainer: Trainer, detectorId: PydanticObjectId, skip:int=0,limit:int=10
):
    try:
        total, sessions =  await trainer.trainSessionList(user,detectorId,skip,limit)
        return TrainSessionListResponse(total=total,sessions=sessions)
...
@router.delete(
    ...
    operation_id="trainerSessionRemove",
    response_model=bool
)
async def session_remove(user:CurrentUser,trainer:Trainer,sessionId:PydanticObjectId):
    try:
        return await trainer.trainSessionRemove(user,sessionId)
    except Exception as e:
        raise HTTPException(status_code=status.HTTP_406_NOT_ACCEPTABLE, detail=str(e))
...
@router.get(
    "/session/detector/running/{detectorId}",
    operation_id="trainerSessionDetectorRunning",
    response_model=bool,
)
async def session_detector_running(
    user: CurrentUser, trainer: Trainer, detectorId: PydanticObjectId
):
    try:
        return await trainer.trainSessionDetectorRunning(user,detectorId)
    except Exception as e:
        raise HTTPException(status_code=status.HTTP_406_NOT_ACCEPTABLE, detail=str(e))
