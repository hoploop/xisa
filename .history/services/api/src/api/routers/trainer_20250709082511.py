...
@router.post(
    "/image/object/to/detector/{detectorId}/{recordId}",
    operation_id="trainerImageObjectToDetector",
    response_model=int,
)
async def image_object_to_detector(
    user: CurrentUser, trainer: Trainer, detectorId: PydanticObjectId, recordId:PydanticObjectId
):
    try:
        return await trainer.trainImageObjectToDetector(user,detectorId,recordId=)
    except Exception as e:
        raise HTTPException(status_code=status.HTTP_406_NOT_ACCEPTABLE, detail=str(e))
