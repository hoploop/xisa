...
class DetectorService(Service, DetectorServicer):
    ...
    async def updateProgress(
        ...
        await self.trainer.trainSessionUpdate(user,trainSessionId,TrainSessionStatus.COMPLETED,results=save_dir)
    ...
    async def trainResult(self, request:TrainResultRequest, context)-> TrainResultResponse:
        file_path = os.path.join(os.getcwd(), self.config.path, request.detector,request.filename)
        try:
