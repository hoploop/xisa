...
class DetectorService(Service, DetectorServicer):
    ...
    async def updateProgress(
        ...
        while results.empty():
            ...
            if not errors.empty():
                err = errors.get()
                await self.trainer.trainSessionUpdate(user,trainSessionId,TrainSessionStatus.ERROR.value,error=err)
            if not logs.empty():
                log.debug(logs.get())
        ...
        await self.trainer.trainSessionUpdate(user,trainSessionId,TrainSessionStatus.COMPLETED.value)
    ...
    async def trainDetector(
        ...
        try:
            ...
            await self.trainer.trainSessionUpdate(user,trainSessionId,TrainSessionStatus.RUNNING.value)
            ...
            asyncio.create_task(
                self.updateProgress(
                    session, user_id, detector, epochs, updates,errors, results, logs,trainSessionId,user
                )
            )
            ...
            log.warning(str(e))
            if trainSessionId:
                await self.trainer.trainSessionUpdate(user,trainSessionId,TrainSessionStatus.ERROR.value,error=str(e))
