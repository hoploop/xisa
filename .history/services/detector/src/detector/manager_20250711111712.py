...
class DetectorManager:
    ...
    def __init__(self,config:DetectorServiceConfig,detector: Detector,storage:StorageClient,trainer:TrainerClient):
    ...
    async def train(self,epochs:int,user:User,session:str,image_size:int):
        ...
        try:
            ...
            path = os.path.join(self.config.path, str(detector.id), self.config.name)
