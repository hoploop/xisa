...
class DetectorManager:
    ...
    async def progress(self,session:str,total:int,value:int,message:Optional[str]=None,error:Optional[str]=None):
        dts = DetectorTrainingSession(detector=self.detector.id,total=total,progress=value,message=message,error=error)
        await self.api.updateSession(session, dts)
        
    async def updateProgress(self,user:User,trainSessionId,session:str,results:Queue,progresses:Queue,errors:Queue):
        err = None
        while results.empty():
            ...
            if not errors.empty():
                err = errors.get()
                await self.progress(session,100,100,err)
                break
        ...
        if err is None:
            ...
            await self.trainer.trainSessionUpdate(
                user, trainSessionId, TrainSessionStatus.ERROR,error=err
            )
    ...
    async def train(self,epochs:int,user:User,session:str,image_size:int):
        ...
        try:
            ...
            if not os.path.exists(path):
                await self.trainer.trainSessionUpdate(
                user, trainSessionId, TrainSessionStatus.ERROR,error="detector.errors.not_found"
            )
    ...
    def trainYOLO(
        ...
        def on_train_epoch_end(trainer):
            ...
            progresses.put((100,60+x,"Epoch: {0}".format(epoch)))
