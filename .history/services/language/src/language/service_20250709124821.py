...
class Translator:
    ...
    def load_translations(self, folder: str):
        ...
        for folder, subs, files in os.walk(folder):
            for file in files:
                ...
                log.debug("Loading translations from: "+file_path)
        ...
        for key in self.translations:
            log.debug("Total translations in {0}: {1}".format(key,self.count(self.translations[key]))))

    def count(self,root) -> int:
        ret = 0
        for key in root:
            if isinstance(root[key],dict):
                ret+= self.count(root[key])
            else:
                ret+=1
        return ret
    ...
    def _get_lang_code(self, root, file, base_folder):
        """
        Determine the language code based on folder structure.
        Assumes: i18n/en/… or i18n/fr/…
        """
        rel_path = os.path.relpath(root, base_folder)
        parts = rel_path.split(os.sep)
        return parts[0] if parts else os.path.splitext(file)[0]
