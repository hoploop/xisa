...
class LanguageService(Service, LanguageServicer):
    ...
    def __init__(self, config: LanguageServiceConfig):
        ...
        self.translator = Translator(os.path.join(os.getcwd(),self.config.i18n)   )
...
#         },
    ...
    async def languageTranslate(self, request:LanguageTranslateRequest, context)-> LanguageTranslateResponse:
        try:
            ...
            if not translationPath[0] in self.translations:
                fpath = os.path.join(os.getcwd(),self.config.i18n)            
                for folder in os.listdir(fpath):
                    if folder == translationPath[0]:
                        self.translations[folder]
                
                raise Exception("Translation cannot be found: "+translationPath[0])
