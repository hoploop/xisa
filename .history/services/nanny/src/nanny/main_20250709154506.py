...
class NannyService:
    ...
    async def check_service(self, name):
        ...
        try:
            ...
            channel = await secure_channel_factory(self.config.security,self.services[name],max_retries=0)
            print("Creating stub")
    ...
    async def monitor_loop(self):
        ...
        while not stop_event.is_set():
            ...
            #asyncio.create_task(self.api.nannyUpdate("nanny",True))
            for name in self.services:
                is_up = await self.check_service(name)
                if is_up:
                    self.statuses[name] = True
                    #asyncio.create_task(self.api.nannyUpdate(name,True))
                    print(f"[OK] {name} is up")
                else:
                    print(f"[DOWN] {name} is down!")
                    self.statuses[name] = False
                    #asyncio.create_task(self.api.nannyUpdate(name,False))
