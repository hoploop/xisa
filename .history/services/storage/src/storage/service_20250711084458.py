...
class StorageService(Service, StorageServicer):
    ...
    async def storageLoad(self, request: StorageLoadRequest, context) -> StorageLoadResponse:
        try:
            data,content_type,size = await self.read_file_by_name(request.folder,request.filename)
            return StorageLoadResponse(status=True,data=data,content_type=content_type,size=size)
        except Exception as e:
            log.warning(str(e))
