        ...
        try:
            ...
            for tio in tios:
                try:
                    ...
                    img = Image.open(BytesIO(self.decode_base64(imageData)))
                    buffer = BytesIO()

                    # Save the image into the buffer in a given format (e.g., PNG, JPEG)
                    img.save(buffer, format="PNG")  # change to "JPEG" if needed

                    # Get the byte data
                    image_bytes = buffer.getvalue()
                    file_id = await self.storage.save(filename,str(detectorId),image_bytes,'image/png')
                    modes = []
                    if tio.test:
