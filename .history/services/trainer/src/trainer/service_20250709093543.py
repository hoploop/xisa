...
from common.models.trainer import TrainImageObject, TrainSession, TrainSessionStatus as ModelTrainSessionStatus
from common.rpc.trainer_pb2 import  TrainImageObjectCountByDetectorRequest, TrainImageObjectCountByDetectorResponse, TrainImageObjectListRequest, TrainImageObjectListResponse, TrainImageObjectRemoveRequest, TrainImageObjectRemoveResponse, TrainImageObjectRequest, TrainImageObjectResponse, TrainImageObjectToDetectorRequest, TrainImageObjectToDetectorResponse, TrainImageObjectUpdateRequest, TrainImageObjectUpdateResponse, TrainSessionCreateRequest, TrainSessionCreateResponse, TrainSessionDetectorRunningRequest, TrainSessionDetectorRunningResponse, TrainSessionListRequest, TrainSessionListResponse, TrainSessionStatus, TrainSessionUpdateRequest, TrainSessionUpdateResponse
...
class TrainerService(Service, TrainerServicer):
    ...
    async def trainImageObjectToDetector(self, request:TrainImageObjectToDetectorRequest, context) -> TrainImageObjectToDetectorResponse:
        try:
            ...
            for tio in tios:
                ...
                imageData = await self.recorder.loadRecordFrameBase64(user,tio.record,tio.frame) ##str base 64
                modes = []
                if tio.test:
    ...
    async def trainSessionCreate(self, request:TrainSessionCreateRequest, context) -> TrainSessionCreateResponse:
        try:
            
            ts = TrainSession(user=PydanticObjectId(request.user),
                         detector=PydanticObjectId(request.detector),
                         status=TrainSessionStatus.IDLE)
            await ts.save()
            return TrainSessionCreateResponse(status=True)
        except Exception as e:
            log.warning(str(e))
            return TrainSessionCreateResponse(status=False,message=str(e))
        
    
    async def trainSessionUpdate(self, request:TrainSessionUpdateRequest, context) -> TrainSessionUpdateResponse:
        try:
            ...
            if found:
                ...
                if request.status == TrainSessionStatus.RUNNING:
                    status = ModelTrainSessionStatus.RUNNING
                elif request.status == TrainSessionStatus.COMPLETED:
                    status = ModelTrainSessionStatus.COMPLETED
                elif request.status == TrainSessionStatus.ERROR:
                    status = ModelTrainSessionStatus.ERROR
                if request.error:
                    found.error = request.error
                found.status = status
                await found.save()
                return TrainSessionUpdateResponse(status=True)
                    
            else:
                TrainSessionUpdateResponse(status=False,message="trainer.errors.session_not_found")
        except Exception as e:
            log.warning(str(e))
            return TrainSessionUpdateResponse(status=False,message=str(e))
        

    async def trainSessionList(self, request:TrainSessionListRequest, context) -> TrainSessionListResponse:
        try:
            sessions = await TrainSession.find(TrainSession.detector==PydanticObjectId(request.detector)).skip(request.skip).limit(request.limit).to_list()
            count =  await TrainSession.find(TrainSession.detector==PydanticObjectId(request.detector)).count()
            ret = []
            for session in sessions:
                ret.append(Conversions.serialize(session))
            return TrainSessionListResponse(status=True,count=count,sessions=ret)
        except Exception as e:
            log.warning(str(e))
            return TrainSessionListResponse(status=False,message=str(e))
        
    
    async def trainSessionDetectorRunning(self, request:TrainSessionDetectorRunningRequest, context) -> TrainSessionDetectorRunningResponse:
        try:
            count = await  TrainSession.find(TrainSession.detecto==PydanticObjectId(request.detector),TrainSession.status==ModelTrainSessionStatus.RUNNING).count()
            return TrainSessionDetectorRunningResponse(status=True,running=count>0)
        except Exception as e:
            log.warning(str(e))
