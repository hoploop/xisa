...
class TrainerService(Service, TrainerServicer):
    ...
    async def trainSessionList(self, request:TrainSessionListRequest, context) -> TrainSessionListResponse:
        try:
            ...
            count =  await TrainSession.find(detector=PydanticObjectId(request.detector)).count()
            ret = []
    ...
    async def trainSessionDetectorRunning(self, request:TrainSessionDetectorRunningRequest, context) -> TrainSessionDetectorRunningResponse:
        try:
            count = await  TrainSession.find(TrainSession.etector=PydanticObjectId(request.detector),status)
        except Exception as e:
            log.warning(str(e))
