...
import { DetectorTrainLabModalComponent } from '@detect/detector-train-lab-modal/detector-train-lab-modal.component';
...
export class DetectorTrainSessionListCardComponent extends BaseComponent implements OnInit {
  ...
  lab(session:TrainSession){
    this.ctx.openModal(DetectorTrainLabModalComponent,{session:session},  {size: 'xl',fullscreen:'lg',centered:true}).subscribe({
      next: (result)=>{},
      error: (result)=>{}
    });
  }
