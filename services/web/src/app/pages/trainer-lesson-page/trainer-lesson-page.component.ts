...
export class TrainerLessonPageComponent extends BaseComponent implements OnInit {
        ...
        this.ctx.api.trainer.trainerLesson(recordId).subscribe({
          next: (result)=>{
            if (result.detector && result._id){
              this.ctx.api.trainer.trainerLessonSetDetector(detectorId,result._id).subscribe({
                next: (resultb)=>{
                  this.lesson = resultb;
