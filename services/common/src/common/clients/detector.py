...
class DetectorClient(Client):
    ...
    async def detectContours(
        ...
        res: DetectContoursResponse = await self.client.detectContours(req)
        if res.status == False:
            raise Exception(res.message)
        ret = []
    ...
    async def uploadDetectorImage(
        ...
        req = UploadDetectorImageRequest(
            user=str(user.id), detector=str(detectorId), file=str(file), modes=rmodes
        )
    ...
    async def loadDetectorLabel(self,user:User,labelId:PydanticObjectId) -> DetectorLabel:
        req = LoadDetectorLabelRequest(user=str(user.id),id=str(labelId))
        res: LoadDetectorLabelResponse = await self.client.loadDetectorLabel(req)
        if res.status == False:
            raise Exception(res.message)
        return Conversions.deserialize(res.label)

    async def trainResult(self,user:User,filename:str) -> TrainResultResponse:
        req = TrainResultRequest(user=str(user.id),filename=filename)
        res: TrainResultResponse = await self.client.trainResult(req)
        return res
    
    async def detectorImage(self,user:User,imageId:PydanticObjectId)-> DetectorImageResponse:
        req = DetectorImageRequest(user=str(user.id),id=str(imageId))
        res: DetectorImageResponse = await self.client.detectorImage(req)
        return  res